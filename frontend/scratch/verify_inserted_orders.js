...
async function main() {
  try {
    ...
    console.log(`\nVerification of the 5 inserted orders:`);
    for (const row of targetOrdersRes.rows) {
      console.log(`- ID: ${row.id} | Name: ${row.customer_name} | Total: ${row.total_amount} VND | Created (Local): ${row.created_at.toLocaleString('vi-VN')}`);
